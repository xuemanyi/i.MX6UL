...
#define IMX6UIRQ_DEVICE	"/dev/imx6uirq"
...
static void print_usage(const char *program)
{
	...
	fprintf(stderr, "Example: %s 1 %s\n", program, IMX6UIRQ_DEVICE);
}

/**
 * parse_count() - 解析需要读取的按键事件数量
...
/**
 * main() - 读取并验证 KEY0 中断事件
 * @argc: 命令行参数个数
 * @argv: 命令行参数数组，可指定事件数量和设备文件路径
 *
 * 默认从 /dev/imx6uirq 阻塞读取一个按键释放事件。每次读到的键值必须为
 * KEY0_VALUE，否则测试失败。
 *
 * Context: 用户进程上下文调用，读取设备时可能阻塞。
 * Return: 全部事件验证成功返回 EXIT_SUCCESS，参数或设备操作失败返回
 * EXIT_FAILURE。
 */
int main(int argc, char *argv[])
{
	const char *device = IMX6UIRQ_DEVICE;
