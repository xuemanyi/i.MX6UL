...
#define LED_DRIVER_NAME             "led_cdev_static"
#define LED_CLASS_NAME              "led_cdev_static_class"
#define LED_NODE_NAME               "led_cdev_static"
...
static int __init led_cdev_static_init(void)
{
    int ret;
    ...
    ret = register_chrdev_region(led_dev, LED_DEVICE_COUNT,
                                 LED_DRIVER_NAME);
    if (ret) {
    ...
    pr_info("%s: registered, major=%d minor=%d\n",
            LED_DRIVER_NAME, LED_MAJOR, LED_MINOR);
...
static void __exit led_cdev_static_exit(void)
{
